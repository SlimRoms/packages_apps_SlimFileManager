...
class InternalMethods
{
    ...
    public static boolean exists(final String file)
    {
        ...
        {
            if (line.toLowerCase().contains("no such") || line.toLowerCase().contains("permission denied")) {
                failed = true;
            } else if (line.trim().equals(file)) {
                return true;
            }
        }

        //try and make this work for directories
        //ls won't return anything on a dir, so assume if we didn't get
        //"No such file or directory" message, that the file/dir exists
        if (!failed) {
            return true;
        } else {
            //reset and try again
        ...
        {
            if (line.toLowerCase().contains("no such")) {
                failed = true;
            } else if (line.trim().equals(file)) {
                return true;
            }
    ...
    static String getMountedAs(String path) throws Exception {
        ...
        Mount rootMount = null;
        if (InternalVariables.mounts != null) {
            for (Mount mount : InternalVariables.mounts)
            {
                RootTools.log("getMountedAs - checking " + mount.getMountPoint().getAbsolutePath());
                if (mount.getMountPoint().getAbsolutePath().equals("/") && !path.equals("/")) {
                    RootTools.log("getMountedAs - skipping / for now");
                    rootMount = mount;
                    continue;
                } else if (path.startsWith(mount.getMountPoint().getAbsolutePath()+"/")
                        || path.equals(mount.getMountPoint().getAbsolutePath())) {
                    RootTools.log("getMountedAs - " + path + " mountedAs - " + (String) mount.getFlags().toArray()[0]);
                    return (String) mount.getFlags().toArray()[0];
                }
            }
            RootTools.log("getMountedAs - checking / as last resort");
            if (rootMount != null && path.contains(rootMount.getMountPoint().getAbsolutePath())) {
                RootTools.log("getMountedAs - / mountedAs - " + (String) rootMount.getFlags().toArray()[0]);
                return (String) rootMount.getFlags().toArray()[0];
            }
